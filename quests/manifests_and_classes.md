...
Rather than classifying this node on the Puppet master and triggering a Puppet agent
...
called a *modulepath* for a module directory matching the class name. The Puppet master then
...
writing our code directly to the modulepath on the Puppet master. In a real production
...
deploy it to your Puppet master. Because we don't have any critical infrastructure
...
Disconnect from the `cowsay.puppet.vm` node to return to your Puppet master.
