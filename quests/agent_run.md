...
Puppet agent communicates with the Puppet master. We'll then write some
...
your infrastructure) act as *Puppet master(s)* and run the Puppet master
service to handle communication with your agents. In a default *monolithic*
master installation, the Puppet master also hosts several supporting
...
All communications between an agent and the Puppet master happen over SSL. Before the
Puppet master communicates with an agent, it needs a way to validate that the agent
...
When the Puppet master service receives a catalog
