...
In this case, we want to include our user accounts on all nodes in our infrastructure, so
we'll be editing the `common.yaml` Hiera data source at
...
data file.
    ...
    cat ~/.ssh/id_rsa.pub >> data/common.yaml
...
With this `profile::pasture::dev_users` class is set up, drop it into the
