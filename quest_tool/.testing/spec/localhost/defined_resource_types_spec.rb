...
describe "Task 2:" do
  it 'Define a simple web_user::user resource type' do
    file("#{MODULE_PATH}web_user/manifests/uesr.pp").content.should match /define\s+web::user/
...
describe "Task 3:" do
  it "Create a test manifest to apply your new defined resource type" do
    file("#{MODULE_PATH}web_user/examples/user.pp").should contain "web_user::\s+{\s*'shelob':\s*}"
  end
end

describe "Task 4:" do
  it "Apply your test manifest" do
    file('/home/shelob').should be_file
...
describe "Task 5:" do
  it "Extend your web_user::user resource type to create a public_html directory and an index.html document" do
    file("#{MODULE_PATH}web_user/manifests/uesr.pp").content.should match /define\s+web::user\s+\{/
    file("#{MODULE_PATH}web_user/manifests/user.pp").content.should match /user\s+{\s+\$title:/
...
describe "Task 7:" do
  it 'Add $content and $password parameters to your defined resource type' do
    file("#{MODULE_PATH}web_user/manifests/uesr.pp").content.should match /\$content\s+=\s+\"\<h1\>Welcome to \${title}'s home page!/
    file("#{MODULE_PATH}web_user/manifests/uesr.pp").content.should match /\$password\s+=\s+undef,/
  end
end

describe "Task 8:" do
  it "Declare a new web_user::user in your test manifest with parameters" do
    file("#{MODULE_PATH}web_user/examples/user.pp").should contain "web_user::\s+{\s*'frodo':"
  end
end
