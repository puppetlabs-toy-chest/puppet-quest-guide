describe "Task 1:" do
  it 'Insert include multi_node into your learning.puppetlabs.vm node declaration' do
    file("#{PROD_PATH}manifests/site.pp")
      .should contain 'include multi_node'
  end
end

describe "Task 2:" do
  it "Run puppet to apply the multi_node class" do
    command('docker ps').stdout
      .should match /webserver/
  end
end
...
describe "Task 3:" do
  it "Install the Puppet agent on your database and webserver nodes" do
    command('docker exec webserver puppet')
      .stdout
      .should match /subcommands/
    command('docker exec database puppet')
      .stdout
      .should match /subcommands/
  end
end
...
describe "Task 5:" do
  it "Sign the certs for your new nodes" do
    file('/etc/puppetlabs/puppet/ssl/ca/signed/database.learning.puppetlabs.vm.pem')
      .should be_file
    file('/etc/puppetlabs/puppet/ssl/ca/signed/webserver.learning.puppetlabs.vm.pem')
      .should be_file
  end
end
