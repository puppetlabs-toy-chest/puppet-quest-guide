...
In theory, you could put whatever bits and pieces of syntactically valid Puppet code you like into a manifest. However, for the broader Puppet architecture to work effectively, you'll need to follow some patterns in how you write your manifests and where you put them. A key aspect of proper manifest management is related to Puppet *classes*.
...
One more note on the topic of classes. In Puppet, classes are *singleton*, which means that a class can only be *once* on a given node. In this sense, Puppet's classes are different than the kind of classes you may have encountered in Object Oriented programming, which are often instantiated multiple times.
...
First, you'll use Puppet to manage the *cowsay* package. Cowsay lets you print a message in the speech bubble of an ascii art cow. It may not be a mission critical software, but it works well as a simple example. Of course, your cow will need something to say. You could supply the messages yourself, but it's more fun to use the *fortune* package to get a randomly generated saying for your cow.
...
Use the `puppet parser` tool to check the syntax of your new manifest:
...
Before applying any changes to your system, it's always a good idea to use the `--noop` flag to do a 'dry run' of the Puppet agent. This will compile the catalog and notify you of the changes that Puppet would have made. 

	puppet apply --noop tests/cowsay.pp

You should see an output like the following:

	Notice: Compiled catalog for learn.localdomain in environment production in 0.62 seconds
...
So you've installed two packages that can work together to do something more interesting than either would do on its own. This is a bit of a silly example, of course, but it's not so different than, say, installing packages for Apache and PHP on a webserver. 
...
At this point, you've already got both packages you want installed on the Learning VM. Go ahead and use a `puppet apply -e` to delete them so you can test the functionality of your new `cowsayings` class:
...
We covered a lot in this quest. We promised manifests and classes, but you got a good taste of how Puppet modules work as well.
...
There are also few details about classes and manifests we haven't gotten to just yet. As you saw in the Power of Puppet, quest, for example, classes can also be declared with *parameters* to customize their functionality.
